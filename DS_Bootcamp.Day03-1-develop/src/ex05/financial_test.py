import httpx
from bs4 import BeautifulSoup
import time
import sys
import pytest

def get_financial_data(ticker, field):

    url = f"https://finance.yahoo.com/quote/{ticker}/financials/"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
    }
    time.sleep(5)
    try:
         with httpx.Client() as client:
            response = client.get(url, headers=headers)
            if response.status_code == 302:
                raise ValueError(f"Тикер '{ticker}' не найден. Проверьте правильность ввода.")
            response.raise_for_status()
        
        
    except httpx.RequestError as e:
        raise Exception(f"Ошибка при подключении к {url}: {e}")
    except httpx.HTTPStatusError as e:
        raise Exception(f"Ошибка HTTP при запросе {url}: {e}")

    soup = BeautifulSoup(response.text, 'html.parser')
    return find(soup, field)
    
def find(soup,field):
    rows = soup.find('div', {'class': 'row lv-0 yf-t22klz'})
    
    
    if not rows:
        raise Exception("Финансовая таблица не найдена на странице.")
    
    row = rows.find_all('div', {'class': 'column'})

    if not row:
        raise Exception(f"Поле '{field}' не найдено.")
    
    return out(row, field)
    
    
def out(row,field):
    values = []
    for col in row:
        
        text = col.get_text().strip()
        if text == field:
            continue

        values.append(text)

    return (field,*values)

def test_first_test():
    result = get_financial_data("MSFT", "Total Revenue")
    expect = ("Total Revenue","254,190,000.00", "245,122,000.00", "211,915,000.00", "198,270,000.00", "168,088,000.00")
    assert result == expect ,f"Ожидалось {expect} , а получили {result}"
    

def test_second_test():
    result = get_financial_data("MSFT", "Total Revenue")
    assert type(result) == tuple,f"Программа возвращает не кортеж"
    

def test_third_test():
    with pytest.raises(ValueError):
        get_financial_data("MSFT1", "Total Revenue")
    

if __name__ == "__main__":

    if len(sys.argv) != 3:
        print("Использование: python financial.py <TICKER> <FIELD>")
        sys.exit(1)
    
    ticker = sys.argv[1]
    field = sys.argv[2]
    
    try:
        
        result = get_financial_data(ticker, field)
        print(result)
        
    except Exception as e:
        print(e)
